  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="relative h-[250px] md:h-[350px] w-full rounded-[2rem] md:rounded-[3rem] overflow-hidden shadow-2xl group border-4 border-white">
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/40 to-transparent flex items-center p-6 md:p-12">
          <div className="max-w-2xl space-y-3">
            <div className="inline-block px-3 py-1 rounded-full bg-white/10 backdrop-blur-md border border-white/20 text-[10px] font-black text-white uppercase tracking-[0.3em]">
      ...
      <div className="relative space-y-4">
        <div className="flex items-center gap-3 bg-white p-3 md:p-4 rounded-[2rem] shadow-xl shadow-gray-100 border border-gray-50">
          <div className="flex-1 relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-300" />
            <input 
              type="text" 
              placeholder={isAdmin ? "Buscar na garagem..." : "Qual máquina você procura hoje?"} 
              className="w-full pl-10 pr-4 py-2 md:py-3 bg-transparent border-none focus:ring-0 text-gray-900 placeholder-gray-400 font-bold text-sm md:text-lg"
              value={searchTerm}
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-5 md:px-8 py-3 md:py-4 rounded-3xl font-black transition-all active:scale-95 shadow-lg text-sm ${isFilterOpen ? 'bg-blue-600 text-white' : 'bg-black text-white'}`}
          >
        ...
        {isFilterOpen && (
          <div className="absolute z-20 w-full bg-white mt-2 p-6 rounded-[2rem] shadow-2xl border border-gray-100 grid grid-cols-1 md:grid-cols-3 gap-6 animate-in slide-in-from-top-4 duration-300">
            <div>
              ...
              <select className="w-full p-3 bg-gray-50 border-none rounded-xl font-bold text-gray-700 outline-none" value={filters.type} onChange={(e) => setFilters({...filters, type: e.target.value})}>
            ...
            <div>
              ...
              <select className="w-full p-3 bg-gray-50 border-none rounded-xl font-bold text-gray-700 outline-none" value={filters.transmission} onChange={(e) => setFilters({...filters, transmission: e.target.value})}>
            ...
            <div>
              ...
              <select className="w-full p-3 bg-gray-50 border-none rounded-xl font-bold text-gray-700 outline-none" value={filters.fuel} onChange={(e) => setFilters({...filters, fuel: e.target.value})}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 pb-20">
        {filteredVehicles.length > 0 ? (
          filteredVehicles.map(vehicle => (
            <VehicleCard key={vehicle.$id} vehicle={vehicle} user={user} isAdmin={isAdmin} />
          ))
        ) : (
