import { Car, Bell, MessageSquare, FileText, LifeBuoy, LogOut, Heart, Clock, LayoutDashboard, ShieldCheck } from 'lucide-react';
import { Link, useLocation } from 'react-router-dom';
import { account } from '../lib/appwrite'; // Importando a configuração que fizemos
...
export default function Sidebar() {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Força o recarregamento para limpar estados do React e voltar ao Login
    ...
    <aside className="w-64 bg-white h-screen flex flex-col border-r border-gray-100 shadow-sm font-sans">
      ...
      <div className="p-6 border-b border-gray-100 flex items-center gap-3">
        <div className="relative flex items-center justify-center w-12 h-12 bg-black rounded-xl shadow-lg shrink-0">
          ...
          {/* Rodas - Animação CSS deve estar no seu index.css */}
      ...
      <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
        {/* Ajustei o path para /dashboard conforme seu App.jsx */}
        <SidebarLink icon={LayoutDashboard} label="Dashboard" to="/dashboard" active={isActive('/dashboard')} />
        <SidebarLink icon={ShieldCheck} label="Painel Admin" to="/admin" active={isActive('/admin')} iconColor="text-orange-600" />
        <SidebarLink icon={Heart} label="Favoritos" to="/favourites" active={isActive('/favourites')} />
      ...
      <div className="p-4 border-t border-gray-100 space-y-2">
        ...
        <SidebarLink icon={FileText} label="Documentos" to="/license" iconColor="text-purple-500" active={isActive('/license')} />
        ...
        <button 
          ...
          className="w-full group flex items-center gap-3.5 px-4 py-3 mt-4 rounded-xl text-sm font-medium transition-all duration-200 text-red-600 hover:bg-red-50"
        >
